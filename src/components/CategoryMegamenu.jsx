...
const categories = [
  { name: 'Fresh Food', subcategories: ['Vegetables', 'Fruits', 'Fresh Seafood', 'Meat'] },
  { name: 'Bakery & Sweets', subcategories: ['Cakes', 'Biscuits', 'Bread', 'Pastries'] },
  { name: 'Dairy & Eggs', subcategories: ['Milk', 'Cheese', 'Yogurt', 'Butter'] },
  { name: 'Pantry', subcategories: ['Grains', 'Spices', 'Oils', 'Canned Goods'] },
  { name: 'Drinks', subcategories: ['Coffee', 'Tea', 'Juice', 'Soft Drinks'] },
  { name: 'Snacks', subcategories: ['Chips', 'Nuts', 'Cookies', 'Candy'] },
]
...
export default function CategoryMegamenu() {
  ...
  return (
    <div className="relative group">
      ...
      {isOpen && (
        <div className="absolute left-0 top-full mt-4 bg-white border border-gray-200 shadow-2xl rounded-xl overflow-hidden w-[700px] z-50">
          <div className="flex h-[450px]">
            ...
            <div className="w-2/3 p-8 bg-white flex flex-col">
              <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-100">
                ...
                <Link to={`/?category=${encodeURIComponent(selectedCategory.name)}`} onClick={() => setIsOpen(false)}>
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-4">
                {selectedCategory.subcategories.map((subcategory) => (
                  <Link
                    key={subcategory}
                    to={`/?category=${encodeURIComponent(selectedCategory.name)}&subcategory=${encodeURIComponent(subcategory)}`}
              ...
              <div className="mt-auto pt-6">
                <div className="bg-gradient-to-r from-red-50 to-orange-50 rounded-xl p-4 border border-red-100 flex items-center justify-between">
                  ...
                  <Link to={`/?category=${encodeURIComponent(selectedCategory.name)}&deals=true`} onClick={() => setIsOpen(false)}>
                    <Button size="sm" className="bg-red-600 hover:bg-red-700 text-white rounded-full text-xs h-8 px-4">Claim Offer</Button>
                  </Link>
                </div>
              </div>
            </div>
