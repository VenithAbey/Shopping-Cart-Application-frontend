      ...
      <div className="bg-red-600 text-white py-2 px-4">
        <div className="max-w-[1400px] mx-auto w-full px-4 flex items-center justify-between text-sm">
          <div className="flex items-center gap-6">
            <Link to="/" className="hover:opacity-80 transition-opacity font-bold">🛒 ShopCart</Link>
            <span className="hidden sm:inline">Fresh products, delivered</span>
          </div>
          <div className="hidden md:flex items-center gap-6">
            <Link to="/orders" className="hover:opacity-80">My Orders</Link>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-[1400px] mx-auto w-full px-4 py-4">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="hidden lg:flex items-center gap-6">
              <CategoryMegamenu />
              <Link to="/?deals=true" className="text-gray-700 font-medium hover:text-red-600 transition-colors">
                Deals
              </Link>
              <button
            ...
            <div className="hidden md:flex items-center gap-4">
              ...
              {authState?.user ? (
                ...
                <Button onClick={() => setShowLoginModal(true)} variant="ghost" className="text-gray-700 hover:bg-gray-100">
                  <User className="w-5 h-5 mr-2" />
                  Sign in
                </Button>
              )}
          ...
          {isMenuOpen && (
            <div className="md:hidden mt-4 space-y-2 pb-4">
              <Link to="/" className="block px-4 py-2 text-gray-700 hover:bg-gray-100 rounded">Categories</Link>
              <Link to="/cart" className="block px-4 py-2 text-gray-700 hover:bg-gray-100 rounded">Cart ({cartCount})</Link>
              {authState?.user ? (
                <>
                  <Link to="/orders" className="block px-4 py-2 text-gray-700 hover:bg-gray-100 rounded">My Orders</Link>
                  <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-gray-700 hover:bg-gray-100 rounded text-sm">
                    Logout
                  </button>
                </>
              ) : (
                <button onClick={() => setShowLoginModal(true)} className="w-full text-left px-4 py-2 text-gray-700 hover:bg-gray-100 rounded text-sm">
                  Sign in
                </button>
              )}
            </div>
      ...
      {showLocationModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-sm text-gray-600 mb-4">Delivery options and delivery speeds may vary for different locations.</p>
            <div className="space-y-4">
              ...
              <Button onClick={handleSaveLocation} disabled={!tempAddress.trim()} className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-2">
                Apply Details
              </Button>
            </div>
          </div>
